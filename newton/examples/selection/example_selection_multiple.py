...
class Example:
    def __init__(self, viewer, num_worlds=16):
        ...
        world.add_builder(arti, xform=wp.transform((0.0, 0.0, 1.0), wp.quat_identity()), update_num_world_count=False)
        world.add_builder(arti, xform=wp.transform((0.0, 0.0, 2.0), wp.quat_identity()), update_num_world_count=False)
        world.add_builder(arti, xform=wp.transform((0.0, 0.0, 3.0), wp.quat_identity()), update_num_world_count=False)
        ...
        self.ants = ArticulationView(
            self.model, "ant", verbose=VERBOSE, exclude_joint_types=[newton.JointType.FREE], squeeze_axes=False
        )
