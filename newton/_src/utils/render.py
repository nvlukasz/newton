...
from ..sim import Model, State, Contacts
        ...
        def populate_shapes(
            ...
            for s in range(shape_count):
                ...
                if geo_hash in geo_shape:
                    ...
                    if geo_type == geometry.GEO_PLANE:
                        # plane mesh
                    ...
                    elif geo_type == geometry.GEO_SDF:
                        continue

                    geo_shape[geo_hash] = shape

                if add_shape_instance and shape_flags[s] & int(geometry.SHAPE_FLAG_VISIBLE):
                    # TODO support dynamic visibility
                    q_shape = X_bs.q
                    if geo_type in (geometry.GEO_CAPSULE, geometry.GEO_CYLINDER, geometry.GEO_CONE):
                        q_shape = X_bs.q * wp.quat_from_axis_angle(wp.vec3(1.0, 0.0, 0.0), -wp.pi / 2.0)
                    if geo_type == geometry.GEO_MESH:
